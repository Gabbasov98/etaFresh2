var FileUploadWithPreview = (function (Q) {
        ...
        constructor(A, g = {}) {
            if (
                ((this.options = {
                    ...
                    text: { browse: p, chooseFile: w, label: d, selectedCount: v },
                }),
                    !A)
            )
                throw new Error(
    ...
    `);
            const u = this.el.querySelector(".custom-file-container .input-hidden"),
                ...
                J = this.el.querySelector(".custom-file-container .clear-button");
            ...
            (this.options.images.baseImage = P ?? this.options.images.baseImage),
                    ...
                    if (g) {
                        if (
                            ...
                            const E = g.getAttribute("data-upload-name"),
                                C = this.cachedFileArray.findIndex(({ name: B }) => B === E);
                            this.deleteFileAtIndex(C);
                        }
                        if (g.matches(".custom-file-container .image-preview-item")) {
                            const E = g.querySelector(".image-preview-item-clear-icon"),
                                C = E == null ? void 0 : E.getAttribute("data-upload-name"),
                                B = this.cachedFileArray.findIndex(({ name: s }) => s === C);
                            if (B < 0) return;
                            const I = {
                                    detail: {
                                ...
                                t = new CustomEvent(e.IMAGE_MULTI_ITEM_CLICKED, I);
        ...
        async addImagesFromPath(A) {
            A.forEach(async (g) => {
                try {
                    ...
                    E instanceof Error && console.warn(`${E.message.toString()}`),
        ...
        addFiles(A) {
            ...
            this.options.multiple || (this.cachedFileArray = []),
                g.forEach((B) => {
                    const I = new File([B], `${B.name || "fallback-name"}${c}${F()}`, {
                        type: B.type,
                    });
                ...
                (g.onload = () => {
                    ...
                    const E = (B) => `
                        <span class="image-preview-item-clear">
                          <span class="image-preview-item-clear-icon" data-upload-name="${B}" data-name="${B.split(':')[0]}">
                            &times;
                          </span>
                        </span>
                      `;
